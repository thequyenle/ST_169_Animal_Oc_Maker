...
class CustomizeActivity : BaseActivity<ActivityCustomizeBinding>() {
            ...
            dataViewModel.allData.collect { list ->
                if (list.isNotEmpty()) {
                    ...
                    if (viewModel.positionSelected == 0) {
                        logCharacterData(list[viewModel.positionSelected], "CUSTOMIZE - dataObservable", "CHARACTER 0")
                    }
    ...
    /**
     * ✅ LOG: Log chi tiết toàn bộ data của character (generic)
     */
    private fun logCharacterData(character: com.example.st169_animal_oc_maker.data.custom.CustomizeModel, source: String, characterName: String) {
        Log.d("CustomizeActivity", "========================================")
        Log.d("CustomizeActivity", "📊 $characterName DATA - $source")
        Log.d("CustomizeActivity", "========================================")
